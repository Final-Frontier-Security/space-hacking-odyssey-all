...
LOCALTGTS := all checkout clean clean-fsw clean-sim clean-gsw code-coverage config debug deploy-sat deploy-gsw deploy-opstation fsw gcov gsw help help-all launch log prep sim stop stop-gsw uninstall
...
build-cryptolib: ## Build CryptoLib on host (needs libgcrypt20-dev); prefer ./scripts/gsw/build_cryptolib.sh
	mkdir -p $(GSWBUILDDIR)
...
help-all: ## Displays advanced help information
	...
	@printf "\t%-20s %s\n" "deploy-sat"     "One-command satellite: prep + build + launch (headless)"
	@printf "\t%-20s %s\n" "deploy-gsw"     "One-command ground station: prep + build OpenC3 + launch"
	@printf "\t%-20s %s\n" "deploy-opstation" "One-command opstation: prep + config + install desktop launcher"
	@printf "\t%-20s %s\n" "start-gsw"      "Launch Ground Software"
	@printf "\t%-20s %s\n" "start-gsw-aws"  "Launch GSW (AWS headless / HTTPS)"
	@printf "\t%-20s %s\n" "start-sat"      "Satellite Launch"
	@printf "\t%-20s %s\n" "start-sat-aws"  "Satellite Launch (AWS published ports)"
	@printf "\t%-20s %s\n" "start-42-opstation" "42 graphics client (opstation → satellite :10001)"

	@echo ""
	@echo "Building:"
	@printf "\t%-20s %s\n" "build-cryptolib" "Build CryptoLib on host (use scripts/gsw/build_cryptolib.sh in Docker)"
	@printf "\t%-20s %s\n" "build-fsw"      "Build the Flight Software (cFS or F')"
	@printf "\t%-20s %s\n" "build-sim"      "Build Simulator"
	@printf "\t%-20s %s\n" "build-test"     "Build Unit Tests"
...
deploy-sat: ## Full satellite setup: prep, config, build, launch (headless)
	$(MAKE) prep
	$(MAKE) config
	$(MAKE) fsw
	$(MAKE) sim
...
deploy-gsw: ## Full ground station setup: config, build OpenC3, launch (no 42 build)
	$(MAKE) config
	chmod +x ./cfg/build/gsw_build.sh ./cfg/build/gsw_launch.sh 2>/dev/null || true
	mkdir -p ./run
	./cfg/build/gsw_build.sh
	./scripts/gsw/setup_ftp.sh
	$(MAKE) start-gsw-aws

deploy-opstation: ## Full opstation setup: prep 42, config, install desktop launcher (does not launch 42)
	mkdir -p ./run/42
	@if [ ! -d "./run/42/.git" ]; then \
...
start-gsw-aws: ## Launch Ground Software (AWS headless / HTTPS)
	./scripts/gsw/launch_gsw_aws.sh

start-sat: ## Satellite Launch
	./scripts/fsw/launch_sat.sh

start-sat-aws: ## Satellite Launch (AWS headless, published ports)
	./scripts/fsw/launch_sat_aws.sh

start-42-opstation: ## 42 graphics client (AWS opstation → SAT_IP:10001)
