...
echo "Prepare 42..."
cd $USER_NOS3_DIR
if [ -d "$USER_NOS3_DIR/42/.git" ]; then
    echo "  Updating existing 42 clone..."
    git -C "$USER_NOS3_DIR/42" fetch --depth 1 origin nos3-main
    git -C "$USER_NOS3_DIR/42" checkout -B nos3-main FETCH_HEAD
else
    git clone https://github.com/nasa-itc/42.git --depth 1 -b nos3-main
fi
$SCRIPT_DIR/apply_42_ipc_patch.py $USER_NOS3_DIR/42 $BASE_DIR/patches/42-nos3-ipc
grep -q 'ServerListenerThread' "$USER_NOS3_DIR/42/Source/42ipc.c" || {
    echo "ERROR: 42 IPC patch was not applied (missing ServerListenerThread)."
    echo "  Ensure $BASE_DIR/patches/42-nos3-ipc exists."
    exit 1
}
cd $USER_NOS3_DIR/42
