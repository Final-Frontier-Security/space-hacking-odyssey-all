...
def strip_ipc_ac_id(lines):
    """Remove legacy AC.ID lines from 42-modified Inp_IPC (incompatible with nos3-main parser)."""
    return [line for line in lines if 'AC.ID for ACS' not in line]
...
# AWS split-host launch scripts (optional mission element)
deploy_cfg = mission_root.find('deploy')
if (deploy_cfg is not None and deploy_cfg.text == 'aws'):
    print('  deploy: aws')
    os.system('cp ./scripts/fsw/launch_sat_aws.sh ./cfg/build/launch.sh')
    if (gsw_cfg == 'openc3'):
        os.system('cp ./scripts/gsw/launch_gsw_aws.sh ./cfg/build/gsw_launch.sh')
if (gsw_cfg == 'cosmos'):
...
if (num_sc < 1):
    ...
    for x in range(1, int(mission_number_spacecraft) + 1):
        ...
        sc_cfdp_en = sc_root.find('components/cfdp/enable').text if sc_root.find('components/cfdp/enable') is not None else 'false'
        ...
        lines = strip_ipc_ac_id(lines)
        with open('./cfg/build/InOut/Inp_IPC.txt', 'w') as fp:
            fp.writelines(lines)

        # AWS: extra TX socket on 10001 for remote 42 graphics (truth42 stays on 9999)
        if (deploy_cfg is not None and deploy_cfg.text == 'aws'):
            with open('./cfg/build/InOut/Inp_IPC.txt', 'r') as fp:
                ipc_lines = fp.readlines()
            n = int(ipc_lines[1].split()[0])
            ipc_lines[1] = str(n + 1) + '                                      ! Number of Sockets\n'
            with open('./cfg/InOut/Inp_IPC.graphics_tx.txt', 'r') as fp:
                gfx_extra = fp.readlines()
            if ipc_lines and not ipc_lines[-1].endswith('\n'):
                ipc_lines[-1] += '\n'
            with open('./cfg/build/InOut/Inp_IPC.txt', 'w') as fp:
                fp.writelines(ipc_lines)
                fp.writelines(gfx_extra)
            print('  Added 42 graphics TX IPC on port 10001 (opstation)')
        ...
        gsw_ip_elem = mission_root.find('gsw-ip')
        if (gsw_ip_elem is not None and gsw_ip_elem.text):
            gsw_ip = gsw_ip_elem.text.strip()
            print('  gsw-ip:', gsw_ip)
            os.system("sed -i 's|<cosmos-hostname>cosmos</cosmos-hostname>|<cosmos-hostname>" + gsw_ip + "</cosmos-hostname>|g' ./cfg/build/sims/nos3-simulator.xml")
