...
THRUSTER_DEVICE_LOOP_COUNT = 1
THRUSTER_DEVICE_DELAY = 3
THRUSTER_DIFF = 0.5
...
def get_generic_thruster_hk()
    cmd("THRUSTER THRUSTER_REQ_HK")
    wait_check_packet("THRUSTER", "THRUSTER_HK_TLM", 1, THRUSTER_RESPONSE_TIMEOUT)
    sleep(THRUSTER_CMD_SLEEP)
end

def generic_thruster_cmd(*command)
    count = tlm("THRUSTER THRUSTER_HK_TLM CMD_COUNT") + 1
    ...
    current = tlm("THRUSTER THRUSTER_HK_TLM CMD_COUNT");
    if (current != count)
        # Try again
        cmd(*command)
        get_generic_thruster_hk()
    ...
    check("THRUSTER THRUSTER_HK_TLM CMD_COUNT >= #{count}")
end

def enable_generic_thruster()
    # Send command
    generic_thruster_cmd("THRUSTER THRUSTER_ENABLE_CC")
    # Confirm
    check("THRUSTER THRUSTER_HK_TLM DEVICE_ENABLED == 'ENABLED'")    
end

def disable_generic_thruster()
    # Send command
    generic_thruster_cmd("THRUSTER THRUSTER_DISABLE_CC")
    # Confirm
    check("THRUSTER THRUSTER_HK_TLM DEVICE_ENABLED == 'DISABLED'")    
end

def safe_generic_thruster()
    get_generic_thruster_hk()
    state = tlm("THRUSTER THRUSTER_HK_TLM DEVICE_ENABLED")
    if (state != "DISABLED")
        generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 0, PERCENTAGE 0")
        generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 1, PERCENTAGE 0")
        generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 2, PERCENTAGE 0")
        generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 3, PERCENTAGE 0")
...
def confirm_generic_thruster_data()
    ...
    generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 0, PERCENTAGE 10")
    sleep THRUSTER_DEVICE_DELAY
    truth_42_GYRO_X_current = tlm("SIM_42_TRUTH SIM_42_TRUTH_DATA WN_0").abs() + THRUSTER_DIFF
    truth_42_GYRO_Y_current = tlm("SIM_42_TRUTH SIM_42_TRUTH_DATA WN_1").abs() + THRUSTER_DIFF
    truth_42_GYRO_Z_current = tlm("SIM_42_TRUTH SIM_42_TRUTH_DATA WN_2").abs() + THRUSTER_DIFF
    generic_thruster_cmd("THRUSTER THRUSTER_PERCENTAGE_CC with THRUSTER_NUMBER 0, PERCENTAGE 0")
...
def confirm_generic_thruster_data_loop()
    THRUSTER_DEVICE_LOOP_COUNT.times do 1
