...
REACTION_WHEEL_TEST_LOOP_COUNT.times do |n|
  ...
  check("REACTION_WHEEL GENRW_HK_TLM_T DEVICE_ERR_COUNT_RW0 == #{dev_err_cnt_RW0+1}")
  check("REACTION_WHEEL GENRW_HK_TLM_T DEVICE_ERR_COUNT_RW1 == #{dev_err_cnt_RW1+1}")
  check("REACTION_WHEEL GENRW_HK_TLM_T DEVICE_ERR_COUNT_RW2 == #{dev_err_cnt_RW2+1}")

  puts("command count is: #{cmd_cnt}")
  check("REACTION_WHEEL GENRW_HK_TLM_T COMMAND_COUNT == #{cmd_cnt}")
  check("REACTION_WHEEL GENRW_HK_TLM_T ERROR_COUNT == #{cmd_err_cnt+3}") #+3 for 3 commands failed
